...
local BrandConfig = {
	Name = "GokuDex",
	Creator = "made gokuthug1",
	Version = "1.1.0",
	Subtitle = "Aura-charged explorer for Roblox worlds",
	OverlayTitle = "GokuDex",
	OverlayDescription = "Dock fast. Search faster. Browse your game tree with an orange-blue energy remaster built to feel native in Roblox.",
	OverlayCredit = "Inspired rebrand by gokuthug1",
	OverlayHighlights = {
		"Dual-pane explorer and properties workflow",
		"Quick layout presets for docked and floating views",
		"Context actions for teleporting, renaming, and jump navigation",
		"Modernized Luau plumbing with fallback API data kept intact"
	},
	Fonts = {
	...
	Theme = {
		Shell = rgb(11,14,21),
		ShellAlt = rgb(17,21,31),
		Surface = rgb(26,31,44),
		SurfaceAlt = rgb(34,40,56),
		Panel = rgb(18,22,32),
		PanelAlt = rgb(24,29,41),
		PanelRaised = rgb(32,38,53),
		Field = rgb(13,17,25),
		FieldBorder = rgb(71,82,103),
		Stroke = rgb(74,84,105),
		StrokeSoft = rgb(49,57,74),
		Text = rgb(245,247,250),
		TextSoft = rgb(187,196,212),
		TextMuted = rgb(132,141,159),
		AccentOrange = rgb(255,152,38),
		AccentOrangeDeep = rgb(219,101,24),
		AccentBlue = rgb(77,172,255),
		AccentBlueDeep = rgb(43,110,232),
		Selection = rgb(255,152,38),
		SelectionAlt = rgb(53,117,225),
		Danger = rgb(255,97,88),
		Success = rgb(104,226,150),
		DockHint = rgb(255,168,77),
		DockIdle = rgb(28,33,48),
		OverlayTint = rgb(10,12,18)
	}
}
...
local RiskyServices = {
	AccountService = true, AnalyticsService = true, AppUpdateService = true,
	AssetService = true, BadgeService = true, BrowserService = true,
	CommerceService = true, CoreGui = false, CoreScriptTestingService = true,
	DataModelPatchService = true, EventIngestService = true, GamePassService = true,
	GoogleAnalyticsConfiguration = true, HttpRbxApiService = true, HttpService = false,
	InternalSyncService = true, LuaWebService = true, MemStorageService = true,
	MessageBusService = true, Omnipresence = true, OpenCloudService = true,
	PackageService = true, PlatformService = true, PlayerDataService = true,
	PolicyService = true, ProcessService = true, PublishService = true,
	RobloxReplicatedStorage = true, ScriptContext = true, SessionService = true,
	SocialService = true, StartPageService = true, Stats = true,
	StopService = true, TeleportService = true, ThirdPartyUserService = true,
	TimerService = true, TweenService = false, UserInputService = false,
	UserService = true, VRService = true, VirtualInputManager = true,
	VoiceChatInternal = true, VoiceChatService = true, WebViewService = true,
}

local Services = setmetatable({}, {
	__index = function(self, ind)
		if RiskyServices[ind] then return nil end
...
local welcomeTitle = welcomeMain:WaitForChild("Title")
local welcomeVersion = welcomeMain:WaitForChild("Version")
local welcomeCreator = welcomeMain:WaitForChild("Creator")
local welcomeCloser = welcomeMain:WaitForChild("Closer")
local welcomeBottomLogo = welcomeBottom:WaitForChild("Logo")
local welcomeBottomDesc = welcomeBottom:WaitForChild("Desc")
local welcomeBottomCredit = welcomeBottom:WaitForChild("Credit")
local welcomeChangelogTitle = welcomeChangelog:WaitForChild("Title")

-- Top Menu References
local topMenu = gui:WaitForChild("TopMenu")
local topMenuVersion = topMenu:WaitForChild("Version")
local topMenuTitle = topMenu:WaitForChild("Title")
local topMenuContent = topMenu:WaitForChild("Content")
local topMenuViewButton = topMenu:WaitForChild("SlideSelect")
local topMenuViewName = topMenuViewButton:WaitForChild("SlideName")
local topMenuViewIcon = topMenuViewButton:WaitForChild("Icon")
...
local API = {}
local RMD = {}
local topMenuContext = nil
local panelMenus = {}
local dockHints = {}
local inlineRenameState = nil
local overlayBuilt = false
...
local ScrollBar do
	...
	function ScrollBar.new(hor)
		...
		scrollThumb.InputBegan:Connect(function(input)
			...
			newMt:Update()
			--while math.abs(mouse[dir] - mouseStart) == 0 do task.wait() end
...
local ContextMenu do
	...
	function ContextMenu.new()
		...
		local function show(self,displayFrame,x,y)
			...
			self.CloseEvent = Services.UserInputService.InputBegan:Connect(function(input)
				if input.UserInputType ~= Enum.UserInputType.MouseButton1 then return end
...
function f.addToPane(window,pane)
	local paneItems = pane == "Left" and LPaneItems or RPaneItems
	local paneContent = pane == "Left" and contentL or contentR
	f.restoreWindow(window)
	f.removeFromPane(window)
	for i,v in pairs(paneItems) do
		if v.Window == window then return end
	end
	for i,v in pairs(paneItems) do
		paneItems[i].Proportion = v.Proportion * 0.8
	end
	window.Parent = paneContent
	if #paneItems == 0 then
		table.insert(paneItems,{Window = window, Proportion = 1})
	else
		table.insert(paneItems,{Window = window, Proportion = 0.2})
	end
	f.buildPanes()
end
...
function f.removeFromPane(window)
	...
	if pane and #pane > 0 then
		...
		if weightTop > 0 and weightBottom == 0 then
			weightTopN = weightTop + pane[windowIndex].Proportion
		elseif weightTop == 0 and weightBottom > 0 then
			weightBottomN = weightBottom + pane[windowIndex].Proportion
		else
			weightTopN = weightTop + pane[windowIndex].Proportion/2
			weightBottomN = weightBottom + pane[windowIndex].Proportion/2
		end
			
		for i = 1,windowIndex-1 do
			pane[i].Proportion = weightTop > 0 and (pane[i].Proportion / weightTop * weightTopN) or pane[i].Proportion
		end
		for i = windowIndex+1,#pane do
			pane[i].Proportion = weightBottom > 0 and (pane[i].Proportion / weightBottom * weightBottomN) or pane[i].Proportion
		end
...
function f.resizePaneItem(window,pane,size)
	...
	if windowIndex > 0 then
		for i = windowIndex+1,#paneItems do
			sizeWeight = sizeWeight + paneItems[i].Proportion
		end
		local oldSize = 1-(sizeWeight+paneItems[windowIndex].Proportion)
		paneItems[windowIndex].Proportion = size
		for i = 1,windowIndex-1 do
			paneItems[i].Proportion = paneItems[i].Proportion / oldSize * (1-(sizeWeight+size))
		end
	end
	f.buildPanes()
end
...
f.fetchAPI = function()
    ...
    if script and script:FindFirstChild("API") then
		...
		if ok then
			...
			f.markLoadIssue("API module", tostring(result))
		end
    end
    if rawAPI == nil then
        ...
        -- Embedded API fallback
    end
	if type(rawAPI) == "string" then
		local decodeOk, decodedApi = pcall(function()
			return Services.HttpService:JSONDecode(rawAPI)
		end)
		if not decodeOk then
			f.markLoadIssue("API decode", tostring(decodedApi))
			rawAPI = {}
		else
			rawAPI = decodedApi
		end
...
f.fetchRMD = function()
    ...
    if script and script:FindFirstChild("RMD") then
		...
		if ok then
			...
			f.markLoadIssue("RMD module", tostring(result))
		end
    end
    if rawRMD == nil then
        ...
        -- Embedded reflection metadata fallback
    end
	if type(rawRMD) == "string" then
		local decodeOk, decodedRmd = pcall(function()
			return Services.HttpService:JSONDecode(rawRMD)
		end)
		if not decodeOk then
			f.markLoadIssue("RMD decode", tostring(decodedRmd))
			rawRMD = {}
		else
			rawRMD = decodedRmd
		end
...
function f.rightClick()
	...
	rightClickContext:Add({Name = "Paste Into", Icon = f.icon(nil,iconIndex.PASTE_ICON), DisabledIcon = f.icon(nil,iconIndex.PASTE_D_ICON), Shortcut = "Ctrl+B", Disabled = #clipboard == 0 or #selection.List == 0, OnClick = function()
		pcall(function()
			for _,v in pairs(selection.List) do
	...
	if f.tabHasChar(selection.List) then
		...
		rightClickContext:Add({Name = "Jump To Player", Icon = "", DisabledIcon = "", Shortcut = "", Disabled = #selection.List == 0, OnClick = function()
			for _,v in pairs(selection.List) do
				local player = f.findPlayerFromObject(v)
				if player then
					f.focusObject(player)
					break
				end
			end
			rightClickContext:Hide()
	...
	f.styleContextMenu(rightClickContext)
...
function f.newExplorer()
	...
	newTree.NodeDraw = function(self,entry,node)
		entry.BackgroundColor3 = BrandConfig.Theme.SelectionAlt
		entry.Indent.EntryName.Font = BrandConfig.Fonts.Body
		...
		if node.Obj.Parent ~= node.Parent.Obj then
			task.spawn(function() f.moveObject(node.Obj,node.Obj.Parent) end)
		end
					
		if self.Selection.Selected[node.Obj] then
			entry.Indent.EntryName.TextColor3 = BrandConfig.Theme.Text
			entry.BackgroundTransparency = 0
		else
			entry.Indent.EntryName.TextColor3 = BrandConfig.Theme.TextSoft
			entry.BackgroundTransparency = 1
		end
	...
	searchBox.Empty.Visible = searchBox.Text == ""
...
function f.gExpanded(obj)
	if not explorerTree or not nodes[obj] then
		return false
	end
	return explorerTree.Expanded[nodes[obj]] and true or false
end
...
local function ensureUICorner(guiObject, radius)
	if not guiObject then return nil end
	local corner = guiObject:FindFirstChild("ThemeCorner")
	if not corner then
		local ok, created = pcall(function()
			local newCorner = Instance.new("UICorner")
			newCorner.Name = "ThemeCorner"
			newCorner.Parent = guiObject
			return newCorner
		end)
		corner = ok and created or nil
	end
	if corner then
		corner.CornerRadius = UDim.new(0, radius or BrandConfig.Radii.Small)
	end
	return corner
end

local function ensureUIStroke(guiObject, name, color, transparency, thickness)
	if not guiObject then return nil end
	...
	if not stroke or not stroke:IsA("UIStroke") then
		local ok, created = pcall(function()
			...
			newStroke.Parent = guiObject
			return newStroke
		end)
		stroke = ok and created or nil
	end
	if stroke then
		stroke.Color = color
...
local function ensureUIGradient(guiObject, name, colors, rotation)
	if not guiObject then return nil end
	...
	if not gradient or not gradient:IsA("UIGradient") then
		...
		gradient = ok and created or nil
	end
	if gradient then
...
local function styleText(guiObject, font, size, color)
	if not guiObject then return end
	guiObject.Font = font or BrandConfig.Fonts.Body
	guiObject.TextSize = size or 14
	guiObject.TextColor3 = color or BrandConfig.Theme.Text
...
local function styleSearchFrame(searchFrame, accentColor)
	...
	searchFrame.BackgroundColor3 = theme.Field
	searchFrame.BorderSizePixel = 0
	ensureUICorner(searchFrame, BrandConfig.Radii.Small)
	ensureUIStroke(searchFrame, "ThemeStroke", theme.FieldBorder, 0.15, 1)
	if searchFrame:FindFirstChild("Entering") then
		searchFrame.Entering.BackgroundColor3 = accentColor
	end
	if searchFrame:FindFirstChild("Search") then
		local searchBox = searchFrame.Search
		searchBox.BackgroundTransparency = 1
		searchBox.ClearTextOnFocus = false
		styleText(searchBox, BrandConfig.Fonts.Body, 14, theme.Text)
		if searchBox:FindFirstChild("Empty") then
			styleText(searchBox.Empty, BrandConfig.Fonts.Body, 13, theme.TextMuted)
		end
	end
	if searchFrame:FindFirstChild("ImageLabel") then
		searchFrame.ImageLabel.Visible = false
	end
end

function f.styleContextMenu(menu)
	if not menu or not menu.Frame then return end
	local theme = BrandConfig.Theme
	local frame = menu.Frame
	frame.BackgroundColor3 = theme.PanelRaised
	frame.BorderSizePixel = 0
	ensureUICorner(frame, BrandConfig.Radii.Medium)
	ensureUIStroke(frame, "ThemeStroke", theme.Stroke, 0.12, 1)
	for _,child in pairs(frame:GetChildren()) do
		if child.Name == "Entry" then
			child.BackgroundColor3 = theme.PanelAlt
			child.BackgroundTransparency = 1
			if child:FindFirstChild("EntryName") then
				styleText(child.EntryName, BrandConfig.Fonts.Body, 14, child.EntryName.TextColor3)
			end
			if child:FindFirstChild("Shortcut") then
				styleText(child.Shortcut, BrandConfig.Fonts.Mono, 13, child.Shortcut.TextColor3)
			end
			if child:FindFirstChild("IconFrame") and child.IconFrame:FindFirstChild("Icon") then
				child.IconFrame.Icon.ImageColor3 = theme.AccentOrange
			end
		elseif child.Name == "Divider" and child:FindFirstChild("Line") then
			child.Line.BackgroundColor3 = theme.StrokeSoft
		end
	end
end
...
local function buildDockHints()
	...
	local theme = BrandConfig.Theme
	local function makeHint(name, xScale, xOffset, align)
		local frame = CreateInstance("Frame",{
			Name = name,
			AnchorPoint = Vector2.new(xScale == 1 and 1 or 0,0),
			BackgroundColor3 = theme.DockIdle,
			BackgroundTransparency = 0.55,
			BorderSizePixel = 0,
			Position = UDim2.new(xScale,xOffset,0,58),
			Size = UDim2.new(0,54,1,-74),
			Visible = false,
			ZIndex = 2,
			Parent = gui
		})
		ensureUICorner(frame, BrandConfig.Radii.Medium)
		ensureUIStroke(frame, "ThemeStroke", theme.Stroke, 0.2, 1)
		...
		styleText(label, BrandConfig.Fonts.Heading, 14, theme.TextMuted)
		return frame
	end
	dockHints.Left = makeHint("DockHintLeft",0,12,Enum.TextXAlignment.Center)
	dockHints.Right = makeHint("DockHintRight",1,-12,Enum.TextXAlignment.Center)
end

function f.updateDockHints()
	buildDockHints()
	local theme = BrandConfig.Theme
	for paneName,hint in pairs(dockHints) do
		...
		if hint:FindFirstChild("Label") then
			hint.Label.Text = active and paneName.." Dock" or "Dock"
			hint.Label.TextColor3 = active and theme.Text or theme.TextMuted
		end
	end
end

function f.restoreWindow(window)
	if not window then return end
	window.Visible = true
...
function f.setOverlayStatus(text, color)
	f.ensureWelcomeOverlay()
	local status = welcomeMain:FindFirstChild("Status")
	if status then
		status.Text = text
...
function f.clearLoadIssues()
	loadIssues = {}
	f.setOverlayStatus(BrandConfig.OverlayDescription, BrandConfig.Theme.TextSoft)
end

function f.markLoadIssue(stepName, message)
	loadIssues[stepName] = message
	f.ensureWelcomeOverlay()
	f.setOverlayStatus(stepName..": "..message, BrandConfig.Theme.Danger)
end

function f.ensureWelcomeOverlay()
	if overlayBuilt then return end
	overlayBuilt = true
	local theme = BrandConfig.Theme
	welcomeFrame.BackgroundColor3 = theme.OverlayTint
	welcomeFrame.BackgroundTransparency = 0.08
	welcomeFrame.BorderSizePixel = 0
	ensureUICorner(welcomeFrame, BrandConfig.Radii.Large)
	ensureUIStroke(welcomeFrame, "ThemeStroke", theme.Stroke, 0.12, 1.5)
	welcomeOutline.Visible = false
	welcomeContents.BackgroundTransparency = 1
	welcomeMain.BackgroundColor3 = theme.Panel
	welcomeMain.BorderSizePixel = 0
	ensureUICorner(welcomeMain, BrandConfig.Radii.Large)
	ensureUIGradient(welcomeMain, "ThemeGradient", {
		ColorSequenceKeypoint.new(0, theme.SurfaceAlt),
		ColorSequenceKeypoint.new(1, theme.Panel)
	}, 90)
	welcomeChangelog.BackgroundColor3 = theme.PanelRaised
	welcomeChangelog.BorderSizePixel = 0
	ensureUICorner(welcomeChangelog, BrandConfig.Radii.Medium)
	ensureUIStroke(welcomeChangelog, "ThemeStroke", theme.StrokeSoft, 0.15, 1)
	welcomeBottom.BackgroundColor3 = theme.PanelAlt
	welcomeBottom.BorderSizePixel = 0
	ensureUICorner(welcomeBottom, BrandConfig.Radii.Medium)
	ensureUIStroke(welcomeBottom, "ThemeStroke", theme.StrokeSoft, 0.15, 1)
	welcomeBottomLogo.Visible = false
	welcomeTitle.Text = BrandConfig.OverlayTitle
	welcomeTitle.TextXAlignment = Enum.TextXAlignment.Left
	welcomeTitle.Position = UDim2.new(0,18,0,22)
	welcomeTitle.Size = UDim2.new(1,-36,0,52)
	styleText(welcomeTitle, BrandConfig.Fonts.Title, 36, theme.Text)
	welcomeVersion.Text = "v"..BrandConfig.Version
	welcomeVersion.Position = UDim2.new(1,-126,1,-26)
	welcomeVersion.Size = UDim2.new(0,112,0,20)
	styleText(welcomeVersion, BrandConfig.Fonts.Mono, 14, theme.AccentBlue)
	welcomeCreator.Text = BrandConfig.Creator
	welcomeCreator.Position = UDim2.new(0,18,1,-26)
	welcomeCreator.Size = UDim2.new(0,180,0,20)
	styleText(welcomeCreator, BrandConfig.Fonts.Heading, 14, theme.TextSoft)
	local subtitle = welcomeMain:FindFirstChild("Subtitle")
	if not subtitle then
		subtitle = CreateInstance("TextLabel",{
			Name = "Subtitle",
			BackgroundTransparency = 1,
			...
			ZIndex = welcomeTitle.ZIndex,
			Parent = welcomeMain
		})
	end
	subtitle.Text = BrandConfig.Subtitle
	styleText(subtitle, BrandConfig.Fonts.Heading, 15, theme.AccentOrange)
	local status = welcomeMain:FindFirstChild("Status")
	if not status then
		status = CreateInstance("TextLabel",{
			Name = "Status",
			BackgroundTransparency = 1,
	...
	styleText(status, BrandConfig.Fonts.Body, 14, theme.TextSoft)
	status.Text = BrandConfig.OverlayDescription
	welcomeProgress.Position = UDim2.new(0,18,0,148)
	welcomeProgress.Size = UDim2.new(1,-36,0,96)
	welcomeProgress.BackgroundTransparency = 1
	if welcomeProgress:FindFirstChild("Line") then
		welcomeProgress.Line.BackgroundColor3 = theme.AccentOrange
	end
	for _,progressLabel in pairs(welcomeProgress:GetChildren()) do
		if progressLabel:IsA("TextLabel") then
			styleText(progressLabel, BrandConfig.Fonts.Body, 14, theme.TextMuted)
		end
	end
	welcomeBottomDesc.Text = BrandConfig.OverlayDescription
	welcomeBottomDesc.Position = UDim2.new(0,16,0,8)
	welcomeBottomDesc.Size = UDim2.new(1,-32,0,24)
	styleText(welcomeBottomDesc, BrandConfig.Fonts.Heading, 16, theme.Text)
	welcomeBottomCredit.Text = BrandConfig.OverlayCredit
	welcomeBottomCredit.Position = UDim2.new(0,16,1,-22)
	welcomeBottomCredit.Size = UDim2.new(1,-32,0,16)
	styleText(welcomeBottomCredit, BrandConfig.Fonts.Body, 12, theme.TextSoft)
	welcomeChangelogTitle.Text = "Energy Boosts"
	welcomeChangelogTitle.Position = UDim2.new(0,14,0,12)
	welcomeChangelogTitle.Size = UDim2.new(1,-28,0,18)
	welcomeChangelogTitle.TextXAlignment = Enum.TextXAlignment.Left
	styleText(welcomeChangelogTitle, BrandConfig.Fonts.Heading, 16, theme.AccentBlue)
	for _,child in pairs(welcomeChangelog:GetChildren()) do
		if child.Name == "Highlight" or child.Name == "Bullet" then
			child:Destroy()
		end
	end
	for i,text in ipairs(BrandConfig.OverlayHighlights) do
		local bullet = CreateInstance("Frame",{
			Name = "Bullet",
			BackgroundColor3 = i % 2 == 0 and theme.AccentBlue or theme.AccentOrange,
			BorderSizePixel = 0,
			Position = UDim2.new(0,14,0,34 + (i - 1) * 32),
			Size = UDim2.new(0,6,0,6),
			ZIndex = welcomeChangelogTitle.ZIndex,
			Parent = welcomeChangelog
		})
		ensureUICorner(bullet, 6)
		local label = CreateInstance("TextLabel",{
			Name = "Highlight",
			BackgroundTransparency = 1,
			Position = UDim2.new(0,28,0,26 + (i - 1) * 32),
		...
		styleText(label, BrandConfig.Fonts.Body, 14, theme.TextSoft)
	end
	welcomeCloser.Text = "X"
	styleText(welcomeCloser, BrandConfig.Fonts.Heading, 16, theme.Text)
	bindHover(welcomeCloser, theme.PanelRaised, theme.Danger, 0.15, 0)
	welcomeCloser.MouseButton1Click:Connect(function()
		f.hideOverlay()
	end)
end

function f.showOverlay(isStartup)
	f.ensureWelcomeOverlay()
	overlayStartup = isStartup and true or false
	overlayVisible = true
	welcomeProgress.Visible = overlayStartup or next(loadIssues) ~= nil
	local statusText = next(loadIssues) and welcomeMain.Status.Text or BrandConfig.OverlayDescription
	if not next(loadIssues) then
		f.setOverlayStatus(statusText, BrandConfig.Theme.TextSoft)
	end
	welcomeFrame.Visible = true
	welcomeFrame.Position = overlayStartup and UDim2.new(0.5,-250,0.5,-180) or UDim2.new(0.5,-250,0,-340)
	welcomeFrame:TweenPosition(UDim2.new(0.5,-250,0.5,-150), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, BrandConfig.Motion.Normal, true)
end

function f.hideOverlay()
	overlayVisible = false
	if welcomeFrame then
		welcomeFrame.Visible = false
		welcomeFrame.Position = UDim2.new(0.5,-250,0,-500)
	end
end
...
function f.applyBaseTheme()
	local theme = BrandConfig.Theme
	f.ensureWelcomeOverlay()
	buildDockHints()
	contentL.BackgroundColor3 = theme.ShellAlt
	contentR.BackgroundColor3 = theme.ShellAlt
	contentL.BackgroundTransparency = 0.25
	contentR.BackgroundTransparency = 0.25
	...
	topMenuViewIcon.Visible = false
	topMenuViewName.Text = currentViewPreset
	topMenuViewName.Position = UDim2.new(0,16,0,0)
	topMenuViewName.Size = UDim2.new(1,-40,1,0)
	styleText(topMenuViewName, BrandConfig.Fonts.Heading, 14, theme.Text)
	topMenuViewArrow.Text = ">"
	topMenuViewArrow.Position = UDim2.new(1,-24,0,0)
	topMenuViewArrow.Size = UDim2.new(0,20,1,0)
	topMenuViewArrow.BackgroundTransparency = 1
	...
	ensureUIStroke(topMenuAboutButton, "ThemeStroke", theme.StrokeSoft, 0.1, 1)
	...
	if entryTemplateRef then
		...
		entryTemplateRef.BorderSizePixel = 0
		if entryTemplateRef:FindFirstChild("Indent") then
			entryTemplateRef.Indent.BackgroundTransparency = 1
			if entryTemplateRef.Indent:FindFirstChild("EntryName") then
				styleText(entryTemplateRef.Indent.EntryName, BrandConfig.Fonts.Body, 14, theme.TextSoft)
			end
		end
	end
	local propEntryTemplate = resources:FindFirstChild("PEntry")
	if propEntryTemplate then
		propEntryTemplate.BackgroundColor3 = theme.PanelRaised
		propEntryTemplate.BorderSizePixel = 0
		if propEntryTemplate:FindFirstChild("Indent") then
			propEntryTemplate.Indent.BackgroundTransparency = 1
			if propEntryTemplate.Indent:FindFirstChild("EntryName") then
				styleText(propEntryTemplate.Indent.EntryName, BrandConfig.Fonts.Body, 14, theme.TextSoft)
			end
			if propEntryTemplate.Indent:FindFirstChild("Control") and propEntryTemplate.Indent.Control:FindFirstChild("String") then
				styleText(propEntryTemplate.Indent.Control.String, BrandConfig.Fonts.Body, 14, theme.Text)
			end
		end
	end
	for _,windowTemplateName in ipairs({"ExplorerPanel","PropertiesPanel"}) do
		f.applyWindowTheme(resources:FindFirstChild(windowTemplateName), windowTemplateName == "ExplorerPanel" and theme.AccentOrange or theme.AccentBlue)
	end
	if resources:FindFirstChild("PropControls") then
		for _,control in pairs(resources.PropControls:GetChildren()) do
			if control:IsA("TextBox") or control:IsA("TextLabel") then
				styleText(control, BrandConfig.Fonts.Body, 14, theme.Text)
			end
			if control:IsA("TextBox") then
				control.BackgroundColor3 = theme.Field
				control.BorderSizePixel = 0
			end
			if control:FindFirstChild("ReadOnly") then
				styleText(control.ReadOnly, BrandConfig.Fonts.Body, 14, theme.TextMuted)
			end
		end
	end
end
...
function f.focusObject(obj)
	if not obj or not nodes[obj] or not explorerTree then return false end
	f.expandAll(obj)
	explorerTree.Selection:Set({obj})
	explorerTree:TreeUpdate()
	explorerTree:Refresh()
	if propertiesTree then
		propertiesTree:TreeUpdate()
...
function f.getVisibleExplorerEntry(obj)
	if not explorerTree or not explorerTree.Entries then return nil end
	for i,entry in pairs(explorerTree.Entries) do
		local node = explorerTree.Tree[i + explorerTree.Index]
		if node and node.Obj == obj then
			return entry,node
		end
	end
	return nil,nil
...
function f.finishInlineRename(shouldCommit)
	if not inlineRenameState or not inlineRenameState.Box then return end
	local renameState = inlineRenameState
	inlineRenameState = nil
	local newName = renameState.Box.Text
	renameState.Box:Destroy()
	if renameState.Label and renameState.Label.Parent then
		renameState.Label.Visible = true
	end
	if shouldCommit and renameState.Obj and newName ~= "" then
		pcall(function()
			renameState.Obj.Name = newName
		end)
	end
	if explorerTree then
		explorerTree:Refresh()
...
function f.startInlineRename(obj)
	obj = obj or f.getPrimarySelection()
	if not obj then return end
	f.focusObject(obj)
	local entry = f.getVisibleExplorerEntry(obj)
	if not entry then return end
	f.finishInlineRename(false)
	local label = entry.Indent.EntryName
	label.Visible = false
	local renameBox = CreateInstance("TextBox",{
		Name = "InlineRename",
	...
	styleText(renameBox, BrandConfig.Fonts.Body, 14, BrandConfig.Theme.Text)
	ensureUICorner(renameBox, BrandConfig.Radii.Small)
	ensureUIStroke(renameBox, "ThemeStroke", BrandConfig.Theme.AccentOrange, 0.05, 1)
	inlineRenameState = {
		Box = renameBox,
		Label = label,
		Obj = obj
	...
	renameBox.FocusLost:Connect(function(enterPressed)
		f.finishInlineRename(enterPressed or renameBox.Text ~= obj.Name)
	end)
	task.defer(function()
...
function f.getCharacterRoot(character)
	if not character then return nil end
	return character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Torso") or character:FindFirstChildWhichIsA("BasePart")
end

function f.getObjectPivot(obj)
...
function f.findPlayerFromObject(obj)
	if not obj then return nil end
	local players = Services.Players
...
function f.detachWindow(window)
	if not window then return end
	if f.checkInPane(window) then
		f.removeFromPane(window)
	else
...
local function tweenPaneFrames(showLeft, showRight)
	contentL:TweenPosition(showLeft and UDim2.new(0,0,0,0) or UDim2.new(0,-explorerSettings.LPaneWidth,0,0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, BrandConfig.Motion.Normal, true)
	contentR:TweenPosition(showRight and UDim2.new(1,-explorerSettings.RPaneWidth,0,0) or UDim2.new(1,0,0,0), Enum.EasingDirection.Out, Enum.EasingStyle.Quart, BrandConfig.Motion.Normal, true)
end

function f.floatWindow(window, position)
	f.restoreWindow(window)
	f.detachWindow(window)
	window.Parent = gui
...
function f.applyViewPreset(name)
	...
	for i = #RPaneItems,1,-1 do
		table.remove(RPaneItems,i)
	end
	if explorerPanel then
		f.restoreWindow(explorerPanel)
		explorerPanel.Parent = gui
	end
	if propertiesPanel then
		f.restoreWindow(propertiesPanel)
		propertiesPanel.Parent = gui
	end
	if currentViewPreset == "Dual Docked" then
		tweenPaneFrames(true,true)
		if explorerPanel then
			explorerPanel.Visible = true
			table.insert(LPaneItems,{Window = explorerPanel, Proportion = 1})
		end
		if propertiesPanel then
			propertiesPanel.Visible = true
			table.insert(RPaneItems,{Window = propertiesPanel, Proportion = 1})
		end
	elseif currentViewPreset == "Explorer Only" then
		tweenPaneFrames(true,false)
		if explorerPanel then
			explorerPanel.Visible = true
			table.insert(LPaneItems,{Window = explorerPanel, Proportion = 1})
		end
	...
	elseif currentViewPreset == "Properties Only" then
		tweenPaneFrames(false,true)
		if explorerPanel then
	...
	elseif currentViewPreset == "Floating" then
		tweenPaneFrames(false,false)
		if explorerPanel then
			explorerPanel.Visible = true
			f.floatWindow(explorerPanel, UDim2.new(0.5,-336,0.5,-210))
		end
		if propertiesPanel then
			propertiesPanel.Visible = true
			f.floatWindow(propertiesPanel, UDim2.new(0.5,16,0.5,-210))
		end
	end
	f.buildPanes()
	if explorerTree then
...
function f.showPanelMenu(window)
	...
	menu:Refresh()
	f.styleContextMenu(menu)
	menu:Show(gui, button.AbsolutePosition.X, button.AbsolutePosition.Y + button.AbsoluteSize.Y + 2)
end
...
function f.initializeChrome()
	...
	topMenuViewButton.MouseButton1Click:Connect(function()
		topMenuContext:Clear()
		for _,preset in ipairs(ViewPresets) do
			local label = preset == currentViewPreset and ("Active: "..preset) or preset
			topMenuContext:Add({
				Name = label,
				Shortcut = "",
				Disabled = false,
				OnClick = function()
					topMenuContext:Hide()
					f.applyViewPreset(preset)
				end
			})
		end
		topMenuContext:Refresh()
		f.styleContextMenu(topMenuContext)
		topMenuContext:Show(gui, topMenuViewButton.AbsolutePosition.X, topMenuViewButton.AbsolutePosition.Y + topMenuViewButton.AbsoluteSize.Y + 2)
	end)
	topMenuAboutButton.MouseButton1Click:Connect(function()
...
function f.updatePropTree(self)
	...
	for i,v in pairs(explorerTree.Selection.List) do
		...
		while class ~= nil and not gotProps[class.Name] do
			for _,prop in pairs(class.Properties) do
				pcall(function()
					local categoryName = prop.Category or "Other"
					if searchText ~= "" then
						local propMatches = string.find(string.lower(prop.Name), searchText, 1, true)
						local categoryMatches = string.find(string.lower(categoryName), searchText, 1, true)
						local classMatches = string.find(string.lower(class.Name), searchText, 1, true)
						if not propMatches and not categoryMatches and not classMatches then
							return
						end
					end
					local newNode = {
						Prop = prop,
						PropCategory = categoryName,
	...
	table.sort(props,function(a,b)
		local categoryA = a.PropCategory or a.Prop.Category or "Other"
		local categoryB = b.PropCategory or b.Prop.Category or "Other"
		local o1 = categoryOrder[categoryA] or 0
		local o2 = categoryOrder[categoryB] or 0
		if o1 ~= o2 then
			return o1 < o2
		else
	...
	local categoryNode
	for i,v in pairs(props) do
		local categoryName = v.PropCategory or v.Prop.Category or "Other"
		if nextCategory ~= categoryName then
...
function f.newProperties()
	...
	newTree.NodeTemplate.BackgroundColor3 = BrandConfig.Theme.PanelRaised
	newTree.NodeTemplate.BorderSizePixel = 0
	newTree.NodeTemplate.Indent.BackgroundTransparency = 1
	newTree.Height = 22
	newTree.OffY = 0
	...
	newTree.NodeDraw = function(self,entry,node)
		...
		if not node.Category then
			...
			--else
				entry.BackgroundColor3 = BrandConfig.Theme.PanelRaised
			--end
		else
			entry.BackgroundColor3 = BrandConfig.Theme.PanelAlt
		end
		...
		if node.Category then
			...
			entry.Indent.EntryName.Font = BrandConfig.Fonts.Heading
			entry.Indent.EntryName.TextColor3 = BrandConfig.Theme.AccentBlue
		else
			entry.Indent.Sep.Visible = true
			entry.Indent.EntryName.Font = BrandConfig.Fonts.Body
			if node.Prop.Tags and node.Prop.Tags["readonly"] then
				entry.Indent.EntryName.TextColor3 = BrandConfig.Theme.TextMuted
			else
				entry.Indent.EntryName.TextColor3 = BrandConfig.Theme.TextSoft
			end
		end
...
function f.newSaveInstancePanel()
	...
	local function createCheckbox(key, labelText, order)
		...
		local box = CreateInstance("TextButton",{
			...
			Text = settingsData[key] and "âœ“" or "",
			Parent = row
		})
		...
		box.MouseButton1Click:Connect(function()
			...
			box.Text = settingsData[key] and "âœ“" or ""
	...
	saveButton.MouseButton1Click:Connect(function()
		...
		task.spawn(function()
			...
			if not saveFunc and (loadstring and (pcall(function() return game.HttpGet end) or request)) then
				...
				local fetchedCode = get("https://raw.githubusercontent.com/luau/UniversalSynSaveInstance/main/saveinstance.luau")
				if fetchedCode then
					local ls = loadstring or (getgenv and getgenv().loadstring)
			...
			local ok, err = pcall(function()
				if type(settingsData) == "table" then
					settingsData.KillAllScripts = false
					settingsData.SafeMode = true
				end
...
local function welcomePlayer()
	...
	f.showOverlay(true)
	task.wait(0.35)
	welcomeProgress:WaitForChild("Progress1").TextColor3 = BrandConfig.Theme.Success
	API = f.fetchAPI()
	welcomeProgress:WaitForChild("Progress2").TextColor3 = BrandConfig.Theme.Success
	RMD = f.fetchRMD()
	welcomeProgress:WaitForChild("Progress3").TextColor3 = BrandConfig.Theme.Success
	task.wait(0.15)
	welcomeProgress:WaitForChild("Progress4").TextColor3 = BrandConfig.Theme.Success
	...
	propertiesTree:Refresh()
	task.wait(0.15)
	welcomeProgress:WaitForChild("Progress5").TextColor3 = BrandConfig.Theme.Success
	f.applyViewPreset("Dual Docked")
	pcall(function()
		f.hideOverlay()
		if welcomeFrame then welcomeFrame.Visible = false end
	end)
end
